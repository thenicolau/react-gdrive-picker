...
- Wildcard MIME type support: pass `mimeTypes={["*"]}` to show all file types from Google Drive without any MIME type filter. Folders are still listed separately. Works in both `<DrivePicker>` component and `useGoogleDrive` hook.
